...
#   
#   d_sigma_1 <- sum(
#     (-(p_1 * dnorm(X, mean = mu_1, sd = sqrt(sigma_1)) / (2 * sigma_1)) +
#        p_1 * dnorm(X, mean = mu_1, sd = sqrt(sigma_1)) * ((X - mu_1)^2 / (2 * sigma_1^2))) / 
#       denominator
#   )
#   
#     d_sigma_2 <- sum(
#     (-((1-p_1) * dnorm(X, mean = mu_2, sd = sqrt(sigma_2)) / (2 * sigma_2)) +
...
X_hat <- hopeHeights %>%
  mutate(p_female = p_1 *dnorm(Height, mu_1, sqrt(sigma_1)) / ( p_1 *dnorm(Height, mu_1, sqrt(sigma_1)) +  (1-p_1) *dnorm(Height, mu_2, sqrt(sigma_2))),
         p_male = (1-p_1) *dnorm(Height, mu_2, sqrt(sigma_2)) / ( p_1 *dnorm(Height, mu_1, sqrt(sigma_1)) +  (1-p_1) *dnorm(Height, mu_2, sqrt(sigma_2)))) %>%
...
The relaxation time is indicative of how long it takes to converge to the stationary distribution, so every 5.59 turns the time remaining to converge decreases by a factor of $1/e \approx 0.368$
